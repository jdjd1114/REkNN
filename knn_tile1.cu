...
int max(int array[],int n){
 ...
 //全局查找KNN
 void global_knn(double * train,double * distance,double * index,int ** labels,int * predict_label,int a,int m,int index_size,int TILE_SIZE,int k,int nclass){
	 int tile_num = (m%TILE_SIZE==0) ? (m/TILE_SIZE):(m/TILE_SIZE + 1);
	 int rank=0;
	 int * count =new int [nclass]; 
	 for (int i=0;i<a;i++){
		 double * q = new double [tile_num];//指向当前每个局部最优的最小值
		 double * p = new double [tile_num];
		 for (int j=0;j<tile_num;j++){
			 p[j] = i + j * TILE_SIZE * a;
			 q[j] = distance[p[j]];
		 }
		 while(rank<k){
			int index = min(q,tile_num);
			labels[i][rank] = index[p[index]];
			rank ++;
			p[index] += a;//指针挪至下一列
			q[index] = distance[p[index]];
		 }
...
int * knn(double * train,int m,int n,double * test,int a,int b,int k,int nclass){
	...
	if (m%(2*TILE_SIZE)>TILE_SIZE){
		...
		int mm = 0;
		mm = (m%TILE_SIZE) > k ? (m%TILE_SIZE);
		//排序
		sort<<<gridSize,blockSize,0,stream0>>>(i_0,gpu_distance0,gpu_index0,a,TILE_SIZE,k);
		sort<<<gridSize,blockSize,0,stream1>>>(i_0+TILE_SIZE,gpu_distance1,gpu_index1,a,mm,k);
		...
		//SAFE_CALL(cudaMemcpyAsync(distance+(i_0+TILE_SIZE)*a, gpu_distance1,a * TILE_SIZE * sizeof(double), cudaMemcpyDeviceToHost, stream1));
		SAFE_CALL(cudaMemcpyAsync(distance+(i_0+TILE_SIZE)*a, gpu_distance1, a * mm * sizeof(double), cudaMemcpyDeviceToHost, stream1));
		SAFE_CALL(cudaMemcpyAsync(index+(i_0+TILE_SIZE)*a, gpu_index1, a * mm * sizeof(double), cudaMemcpyDeviceToHost, stream1));
	...
	if((m%(2*TILE_SIZE) != 0) && (m%(2*TILE_SIZE)<=TILE_SIZE)){
		...
		//排序
		sort<<<gridSize,blockSize,0,stream0>>>(i_0,gpu_distance0,gpu_index0,a,m%TILE_SIZE,k);
		
		int mm = 0;
		mm = (m%TILE_SIZE) > k ? (m%TILE_SIZE);
		//复制结果到CPU端
		SAFE_CALL(cudaMemcpyAsync(distance+i_0*a, gpu_distance0, a * mm * sizeof(double), cudaMemcpyDeviceToHost, stream0));
		SAFE_CALL(cudaMemcpyAsync(index+i_0*a, gpu_index0, a * mm * sizeof(double), cudaMemcpyDeviceToHost, stream0));
	}
	...
	global_knn(train,distance,labels,a,index_size,TILE_SIZE,k);
	start = clock();
	/*
